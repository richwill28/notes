...
\begin{proof}
  ...
  \begin{example}
    \explabel{mso-of-dfa}
    \figref{visit-record-example} shows a DFA, its run on the word $w = aabbb$, and the visit record $R_w$. Observe that each position belongs to the visit record of exactly one state. In other words, $R_w(q_0)$, $R_w(q_1)$, and $R_w(q_2)$ form a partition of the set of positions $\set{1, 2, \dots, 5}$.
  \end{example}
  ...
  \begin{example}
    For the DFA of \figref{visit-record-example} with states $\set{q_0, q_1, q_2}$, we get
    \begin{align*}
      & \Init(X_0, X_1, X_2) = \\
