...
There are multiple definitions of the time hierarchy theorem. We present the following one by Sipser \cite{sipser2013introduction}, which is more intuitive.
...
\begin{remark}
  \corref{alternative-definition-of-time-hierarchy-theorem} is very often presented as the time hierarchy theorem in other literature.
\end{remark}
...
\todo{State the fact that our time hierarchy theorem is derived from simulating a single-tape TM with another single-tape TM. Sharper results can be achieved if we use multi-tape TMs. We explore the results here.}
