...
Common sense suggests that giving a Turing machine more time should increase the class of problems that it can solve. For example, Turing machines should be able to decide more languages in time $n^3$ than they can in $n^2$. The \vocab{time hierarchy theorem} proves that this intuition is correct, subject to certain conditions described below. We use the term \emph{hierarchy theorem} because this theorem proves that time complexity classes aren't all the same—they form a hierarchy whereby the classes with larger bounds contain more languages than do the classes with smaller bounds.
...
When designing algorithms of arbitrary time complexity $f : \NN \to \NN$, we need to make sure that the algorithm does not exceed the time bound. Furthermore, when invoked on input $w$, the algorithm is not given the time bound $f(\len{w})$ explicitly. A reasonable design methodology is to have the algorithm compute this bound before doing anything else. This, in turn, requires the algorithm to read the entire input and compute $f(n)$ in $O(f(n))$ steps; otherwise, this preliminary stage already consumes too much time. The latter requirement motivates the notion of time-constructible functions.

\begin{theorem}[Time hierarchy theorem]
  \thmlabel{time-hierarchy-theorem}
  For any time-constructible function $f : \NN \to \NN$, a language $A$ exists that is decidable in $O(f(n))$ time but not decidable in time $o(f(n) / \log f(n))$.
\end{theorem}

The time hierarchy theorem for deterministic multi-tape Turing machines was first proven by Stearns and Hartmanis \cite{hartmanis1965computational}. It was improved some time later when Hennie and Stearns improved the efficiency of the universal Turing machine \cite{hennie1966two}.

We present the following proof by Sipser \cite{sipser2013introduction}, with slight modification.
...
\begin{proofidea}
  ...
  We can fix this problem by modifying $D$ to give it additional opportunities to avoid $M$'s language. Instead of running $M$ only when $D$ receives input $\enc{M}$, it runs $M$ whenever it receives an input of the form $\enc{M}\texttt{1}\texttt{0}^\ast$, that is, an input of the form $\enc{M}$ followed by a $\texttt{1}$ and some number of $\texttt{0}$s. In other words, we get to simulate $M$ on infinitely many $w$. Then, if $M$ really is running in $o(f(n) / \log f(n))$, we will eventually encounter a sufficiently large $w$, where $w = \enc{M}\texttt{1}\texttt{0}^k$ for some large value of $k$, and $D$ will have enough time to run it to completion because the asymptotic must eventually kick in.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & \ \quad \text{2. } \text{Compute $f(n)$ using time constructibility, and store the value} \\
    & \ \quad \phantom{\text{2. }} \text{$\ceil{f(n) / \log f(n)}$ in a binary counter. Decrement this counter} \\
    & \ \quad \phantom{\text{2. }} \text{before each step used to carry out stages 3, 4, and 5. If the} \\
    & \ \quad \phantom{\text{2. }} \text{counter ever hits 0, \emph{reject}.} \\
    & \ \quad \text{3. } \text{If $w$ is not of the form $\enc{M}\texttt{1}\texttt{0}^\ast$ for some TM $M$, \emph{reject}.} \\
...
\begin{corollary}
  For any two time-constructible functions $f, g : \NN \to \NN$ where $f(n) = o(g(n) / \log g(n))$,
...
\begin{align*}
  D = & \ \text{``On input $w$:} \\
  & \ \quad \text{1. } \text{Let $n$ be the length of $w$.} \\
  & \ \quad \text{2. } \text{Compute $f(n)$ using time constructibility, and store the value $f(n)$} \\
  & \ \quad \phantom{\text{2. }} \text{in a binary counter. Decrement this counter before each step used} \\
  & \ \quad \phantom{\text{2. }} \text{to carry out stages 3, 4, and 5. If the counter ever hits 0, \emph{reject}.} \\
  & \ \quad \text{3. } \text{If $w$ is not of the form $\enc{M}\texttt{1}\texttt{0}^\ast$ for some TM $M$, \emph{reject}.} \\
  & \ \quad \text{4. } \text{Nondeterministically simulate $M$ on $w$.} \\
  & \ \quad \text{5. } \text{If $M$ accepts, then \emph{reject}. \incorrect{If $M$ rejects, then \emph{accept}.}''}
\end{align*}
