...
\begin{proofidea}
  ...
  \begin{center}
    \includegraphics[width=0.7\textwidth]{media/turing_machine_intervals.jpg}
  \end{center}
  ...
  \begin{center}
    \includegraphics[width=0.8\textwidth]{media/lazy_diagonalization.jpg}
  \end{center}
  If we assume the contrary that $M_k$ accepts the same language as $D$, then we get the following new equivalence class.
  \begin{center}
    \includegraphics[width=0.8\textwidth]{media/lazy_diagonalization_contradiction.jpg}
  \end{center}
...
\begin{remark}
  Notice that the $1 / \log f(n)$ factor does not appear in our hierarchy theorem, unlike in its deterministic counterpart. This absence is due to the efficient simulation of an NTM by another NTM, which only incurs a constant factor to the time complexity \cite{arora2009computational}.
\end{remark}
