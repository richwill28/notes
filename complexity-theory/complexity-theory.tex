...
\begin{proof}
  Once again, we define the following NTM $D$ that decides a language $A$ that is not decidable in $O(f(n))$ time.
  \begin{align*}
  ...
  \end{align*}
  It is easy to see that $D$ runs within $O(g(n))$ time. Now, we show that $A$ is not decidable in $O(f(n))$ time. Assume the contrary that some TM $M_A$ decides $A$ in $O(f(n))$ time. Consider an input of the form $w = (M_A, \texttt{1}^k, \epsilon)$ for sufficiently large $k$. We have:
  \begin{align*}
