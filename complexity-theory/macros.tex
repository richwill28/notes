% General

\newenvironment{automata}{
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto,initial text=$ $]}{
\end{tikzpicture}}

\newenvironment{proofidea}{
\begin{proof}[Proof Idea]}{
\end{proof}}

\newenvironment{turing}[2]{
  \vspace{\parskip}
  \begin{addmargin}[2.5em]{2.5em}
    \setlength{\parskip}{0.25em}
    #1 = #2
    \begin{enumerate}[leftmargin=5em,label=\arabic*.]
      }{
    \end{enumerate}
\end{addmargin}\vspace{\parskip}}

\newcommand{\dubious}[1]{\sethlcolor{BurntOrange}\hl{#1}}
\newcommand{\highlight}[1]{\sethlcolor{yellow}\hl{#1}}
\newcommand{\incorrect}[1]{\textcolor{red}{#1}}
\newcommand{\vocab}[1]{\textbf{\color{blue} #1}}

\newcommand{\chplabel}[1]{\label{chp:#1}}
\newcommand{\deflabel}[1]{\label{def:#1}}
\newcommand{\corlabel}[1]{\label{cor:#1}}
\newcommand{\lemlabel}[1]{\label{lem:#1}}
\newcommand{\prblabel}[1]{\label{prb:#1}}
\newcommand{\prplabel}[1]{\label{prp:#1}}
\newcommand{\seclabel}[1]{\label{sec:#1}}
\newcommand{\thmlabel}[1]{\label{thm:#1}}

\newcommand{\chpref}[1]{Chapter~\ref{chp:#1}}
\newcommand{\defref}[1]{Definition~\ref{def:#1}}
\newcommand{\corref}[1]{Corollary~\ref{cor:#1}}
\newcommand{\lemref}[1]{Lemma~\ref{lem:#1}}
\newcommand{\prbref}[1]{Problem~\ref{prb:#1}}
\newcommand{\prpref}[1]{Proposition~\ref{prp:#1}}
\newcommand{\secref}[1]{Section~\ref{sec:#1}}
\newcommand{\thmref}[1]{Theorem~\ref{thm:#1}}

% Computer Science
\newcommand{\coNPTIME}{\textbf{coNP}}
\newcommand{\DLOGTIME}{\textbf{DLOGTIME}}
\newcommand{\DSPACE}{\textbf{DSPACE}}
\newcommand{\DTIME}{\textbf{DTIME}}
\newcommand{\EXPTIME}{\textbf{EXPTIME}}
\newcommand{\IP}{\textbf{IP}}
\newcommand{\NEXPTIME}{\textbf{NEXPTIME}}
\newcommand{\NPSPACE}{\textbf{NPSPACE}}
\newcommand{\NPTIME}{\textbf{NP}}
\newcommand{\NTIME}{\textbf{NTIME}}
\newcommand{\PCP}{\textbf{PCP}}
\newcommand{\PSPACE}{\textbf{PSPACE}}
\newcommand{\PTIME}{\textbf{P}}

\newcommand{\threeSAT}{\text{3SAT}}
\newcommand{\HALT}{\text{HALT}}
\newcommand{\SAT}{\text{SAT}}
\newcommand{\TQBF}{\text{TQBF}}
\newcommand{\USAT}{\text{USAT}}

\newcommand{\karp}{\leq_m^P}
\newcommand{\oracle}{\mathcal{O}}
\newcommand{\pad}{\text{pad}}
\newcommand{\poly}{\text{poly}}

\newcommand{\enc}[1]{\left< #1 \right>}

% Mathematics
\newcommand{\abs}[1]{| #1 |}
\newcommand{\ceil}[1]{\lceil #1 \rceil}
\newcommand{\concat}{\cdot}
\newcommand{\floor}[1]{\lfloor #1 \rfloor}
\newcommand{\len}[1]{| #1 |}
\newcommand{\set}[1]{\{ #1 \}}
\newcommand{\size}[1]{| #1 |}
\renewcommand{\emptyset}{\varnothing}
\renewcommand{\phi}{\varphi}
\renewcommand{\Pr}{\textbf{Pr}}

\newcommand{\CC}{\mathbb C}
\newcommand{\FF}{\mathbb F}
\newcommand{\NN}{\mathbb N}
\newcommand{\QQ}{\mathbb Q}
\newcommand{\RR}{\mathbb R}
\newcommand{\ZZ}{\mathbb Z}
