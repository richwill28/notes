...
\begin{proof}{33}
  A rational number is any number that can be expressed in the
  form $p/q$, where $p$ and $q$ are integers. Thus, what the theorem
  asserts is that no matter how $p$ and $q$ are chosen, it is never the
  case that $(p/q)^2 = 2$. The line of attack is indirect, using a type
  of argument referred to as a proof by contradiction. The idea is to
  assume that there is a rational number whose square is 2 and then
  proceed along logical lines until we reach a conclusion that is
  unacceptable. At this point, we will be forced to retrace our steps
  and reject the erroneous assumption that some rational number squared
  is equal to 2. In short, we will prove that the theorem is true by
  demonstrating that it cannot be false.
...
\begin{theorem}[Cauchy-Schwarz inequality]
  If $a_1, \dots, a_n$ and $b_1, \dots, b_n$ are arbitrary real
  numbers, we have
...
\begin{example}
  ...
  \begin{itemize}
    ...
    \item $\sup(\set{\frac{n}{n + 1} : n \in \NN}) = 1$;
      ...
      that the infimum here is in the set, while the supremum is
      not in the set.
...
\begin{proofsketch}{78}
  ...
  correspond to a unique cut, and each cut corresponds to a unique
  real number.
...
\begin{definition}[Closed and open intervals]
  ...
  intervals and intervals to $\pm\infty$ are again exactly as you
  would expect.
\end{definition}
