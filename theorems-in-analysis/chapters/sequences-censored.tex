...
\begin{definition}[$\epsilon$-neighborhood]
  Given a real number $a \in \RR$ and a positive number $\epsilon >
  0$, the set
...
\begin{proof}{49}
  ...
  But since $a < b$, we must have $b + 2a < 2b + a$. But this then
  implies that
...
\begin{proof}{32}
  ...
  sequence squeeze theorem (\thmref{sequence-squeeze}) we see that
  $\lim_{n \to
...
\begin{proof}{38}
  ...
  $(a_n)$, which is monotone increasing, we have $a_n \leq
  a_{n_k}$. So we have
  \[ a_n \leq a_{n_k} \leq \sup(\{a_{n_k} : k \in \NN\}), \]
  which proves that $(a_n)$ is bounded above (by this supremum), and
  so by the monotone convergence theorem we have proven that
  $(a_n)$ converges.
